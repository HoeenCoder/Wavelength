...
exports.commands = {
	...
	ontime: function (target, room, user) {
		...
		if (isConnected) {
			this.sendReplyBox(SG.nameColor(user, true) + '\'s total ontime is <b>' + displayTime(convertTime(totalOntime)) + '</b>.' + ' Current ontime: <b>' + displayTime(convertTime((currentOntime))) + '</b>.');
		} else {
			this.sendReplyBox(SG.nameColor(user, true) + '\'s total ontime is <b>' + displayTime(convertTime(totalOntime)) + '</b>.' + ' Currently not online.');
		}
	},
};
