...
exports.commands = {
	fc: {
		...
		help: function (target, room, user) {
			this.sendReplyBox('<h2> Friend Code Commands: </h2><br />' + '<i> /fc make </i> - Makes your friend code. <br />' + '<i> /fc get </i> - Displays your friend code (in case you forget.)');
		},
	},
	friends: 'friend',
	friend: {
		add: function (target, room, user) {
			...
			if (!targetUser || !targetCode) return this.errorReply('/friend add [friend], [fc]');
			...
			Friend.addFriend(targetUser, targetCode, user);
		},
		...
		list: function (target, room, user) {
			...
			if (!amount) {
				...
				return this.sendReply(`You have ${amount} friend${Chat.plural(amount)}: ${array.join(', ')}`);
			}
		},
		help: function (target, room, user) {
			this.sendReplyBox('<h2> Friends Commands: </h2> <br />' + '<i> /friend add [target], [friend code] </i> - Adds a friend. <br />' + '<i> /friend remove [target] </i> - Removes a friend. <br />' + '<i> /friend list </i> - Lists all your friends. <br />' + '<i> /friend removeall </i> - Removes all friends.');
		},
	},
};
