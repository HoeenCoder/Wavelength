...
const ESHOP_PATH = 'config/eShop.json';
...
function writeShop() {
	...
	FS(ESHOP_PATH).writeUpdate(() => JSON.stringify(WL.eShop));
}
...
try {
	let raw = JSON.parse(FS(ESHOP_PATH).readIfExistsSync());
	WL.eShop = raw;
} catch (e) {
	writeShop();
...
//Usage notification
if (FS('server/wavelength-plugins/shop-private.js').readIfExistsSync()) {
