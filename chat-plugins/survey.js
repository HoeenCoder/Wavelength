...
class Survey {
	...
	generateQuestion() {
		let output = '<div class="infobox"><p style="margin: 2px 0 5px 0"><span style="border:1px solid #6A6;color:#484;border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> Survey</span> <strong style="font-size:11pt">' + (this.allowHTML ? this.question : Tools.escapeHTML(this.question)) + '</strong></p>';
	...
	generateResults(ended) {
		...
		let output = '<div class="infobox"><p style="margin: 2px 0 5px 0">' + icon + ' <strong style="font-size:11pt">' + (this.allowHTML ? this.question : Tools.escapeHTML(this.question)) + '</strong></p>';
		let ownReply = null;
