...
function validate(me, targetUser, quiet) {
  ...
  if (!species.exists || !species.learnset || species.gen < 1) {
    valid = false;
...
function buildMenu(userid) {
  ...
  let output = '';
  output += '<div class="setchart" style="height: 155px; background-image:url(//play.pokemonshowdown.com/sprites/' + (Tools.getTemplate(toId(SG.ssb[userid].species)).gen === 7 ? 'bw' : 'xydex') + '' + (SG.ssb[userid].shiny ? '-shiny' : '') + '/' + SG.ssb[userid].species.toLowerCase() + '.png); background-position: -2px -3px; background-repeat: no-repeat;">';
