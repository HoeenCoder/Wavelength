...
var customMovepool = ['Stretch', 'Flame Tower', 'Rain Spear', 'Healing Herbs', 'Electro Drive', 'Hailstorm', 'Beat Down', 'Nuclear Waste', 'Terratremor', 'Ventilation', 'Psychic Sheild', 'Swarm Charge', 'Rock Cannon', 'Spook', 'Imperial Rampage', 'Shadow Run', 'Magnorang', 'Majestic Dust']; //Add defual custom move names here.
...
function buildMenu(userid) {
  ...
  let output = '';
  output += '<div class="setchart" style="background-image:url(//play.pokemonshowdown.com/sprites/xydex' + (SG.ssb[userid].shiny ? '-shiny' : '') + '/' + toId(SG.ssb[userid].species) + '.png); background-position: -2px -3px; background-repeat: no-repeat;">';
